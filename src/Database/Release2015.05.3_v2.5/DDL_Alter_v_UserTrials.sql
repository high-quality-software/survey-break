...
/****** Object:  View [dbo].[v_UserTrials]    Script Date: 04/27/2015 16:25:42 ******/
SET ANSI_NULLS ON
GO
...
-- Modified by gio on 4/27/2015 to correct third use case by selecting the usertype_ID rather than hardcoding the value.  
--
...
-- USAGE: UserAlias = 'NTUserID' OR [User_ID] = 123 (User's PK) 
	...
	SELECT  t.Trial_ID,
			...
			u.FName,
			u.UserAlias,
			(SELECT UserType_ID FROM UserType WHERE UserTypeName = 'LeadSRR')
	FROM TrialCatalog t
